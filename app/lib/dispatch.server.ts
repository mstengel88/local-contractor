...
function findPhoneInLooseText(value: string) {
...
function findPhoneInAddressBlocks(lines: string[]) {
  const starts = lines
    .map((line, index) => ({ line, index }))
    .filter(({ line }) => /^(billing|shipping)\s+address\b/i.test(line))
    .map(({ index }) => index);
  ...
  for (const start of starts) {
    ...
    for (const line of lines.slice(start, start + 12)) {
      if (
        block.length &&
        /^(billing|shipping)\s+address\b|what happens next\??|order summary|subtotal|total:?$/i.test(line)
      ) {
        break;
      }
      block.push(line);
    ...
    const blockPhone =
      findPhoneNearEmail(block) ||
      findPhoneInText(block.join(" ")) ||
      findPhoneInLooseText(block.join(" "));
    if (blockPhone) return blockPhone;
  }
