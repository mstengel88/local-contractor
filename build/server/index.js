...
function RouteMapPreview({
  ...
  const routePlan = useMemo(() => routes2.map((route35) => ({
    ...
    stops: route35.orders.map((order) => [order.address, order.city].map((part) => String(part || "").trim()).filter(Boolean).join(", ")).filter(Boolean)
    ...
    async function drawMap() {
      ...
      try {
        ...
        const drawFallbackRoute = async (route35) => {
          const points = (await Promise.all([originAddress, ...route35.stops, originAddress].map((address) => geocodeAddress(address)))).filter(Boolean);
          ...
          points.forEach((point, index) => {
            const marker = new google.maps.Marker({
              ...
              title: index === 0 || index === points.length - 1 ? "Yard" : route35.stops[index - 1]
            });
