...
const customQuote = UNSAFE_withComponentProps(function PublicCustomQuotePage() {
  ...
  const quoteText = useMemo(() => {
    ...
    const linesText = ((_a3 = actionData.selectedLines) == null ? void 0 : _a3.map((line) => `${line.title}: $${(Number(line.price || 0) * Number(line.quantity || 0)).toFixed(2)}`).join("\n")) || "";
    return [linesText, `Products: $${Number(actionData.pricing.productsSubtotal).toFixed(2)}`, `Delivery: $${Number(actionData.pricing.deliveryAmount).toFixed(2)}`, `Tax: $${Number(actionData.pricing.taxAmount).toFixed(2)}`, `TOTAL: $${Number(actionData.pricing.totalAmount).toFixed(2)}`].filter(Boolean).join("\n");
  }, [actionData]);
